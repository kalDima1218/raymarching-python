...
class Sphere(SDF):
	...
	def get_formula(self):
		return "(sqrt(x**2 + y**2 + z**2) - " + str(self.r) + ")"

	def get_formula_cpp(self):
		return "(my_sqrt(sqpow(x) + sqpow(y) + sqpow(z)) - " + str(self.r) + ")"
...
class Cube(SDF):
	...
	def get_formula(self):
		return "(max(abs(x), abs(y), abs(z)) - " + str(self.s/2) + ")"
		
	def get_formula_cpp(self):
		return "(my_max(my_abs(x), my_abs(y), my_abs(z)) - " + str(self.s/2) + ")"
...
class Tor(SDF):
	...
	def get_formula(self):
		return "(sqrt((sqrt(x**2 + z**2) - " + str(self.B_R) + ")**2 + y**2) - " + str(self.r) + ")"
		
	def get_formula_cpp(self):
		return "(my_sqrt(sqpow(my_sqrt(sqpow(x) + sqpow(z)) - " + str(self.B_R) + ") + sqpow(y) - " + str(self.r) + ")"
...
class obj_plus_obj:
	...
	def get_formula(self):
		return "min(" + self.obj1.get_formula() + ", " + self.obj2.get_formula() + ")"
		
	def get_formula_cpp(self):
		return "my_min(" + self.obj1.get_formula_cpp() + ", " + self.obj2.get_formula_cpp() + ")"
...
class obj_minus_obj:
	...
	def get_formula(self):
		return "max(" + self.obj1.get_formula() + ", " + self.obj2.get_formula() + ")"
		
	def get_formula_cpp(self):
		return "my_max(" + self.obj1.get_formula_cpp() + ", " + self.obj2.get_formula_cpp() + ")"
...
def formula(x, y, z):
	return my_min(my_min(my_min(my_sqrt(sqpow(my_sqrt(sqpow(x) + sqpow(z)) - 0.3) + sqpow(y - 0.18)) - 0.02, my_sqrt(sqpow(x) + sqpow(y) * 2.5 + sqpow(z)) - 0.4), my_min(my_max(my_max(x + y - 0.15 - 0.05 - 0.5, - (y) + 0.19 - 0.1), my_sqrt(sqpow(my_sqrt(sqpow(x - 0.55) + sqpow(y - 0.09)) - 0.1) + sqpow(z - 0.1)) - 0.04), my_max(-(- (y) + 0.19 - 0.1), my_sqrt(sqpow(my_sqrt(sqpow(x - 0.35) + sqpow(y - 0.09)) - 0.1) + sqpow(z - 0.1)) - 0.04))), my_sqrt(sqpow(x) + sqpow(y - 0.27) + sqpow(z)) - 0.05)

		
class render:
	def __init__(self, w, h, a, b, cam_dist, max_dist, sdf: Callable[[float, float, float], float]=lambda x, y, z: 0.0):
		self.w = w
		self.h = h
		self.a = a
		self.b = b
	...
	def raymarch_light(self, start_pos, ray_dir, k):
		dist_to_light = sqrt((self.light_pos[0] - start_pos[0])**2 + (self.light_pos[1] - start_pos[1])**2 + (self.light_pos[2] - start_pos[2])**2)
	...
	def rendering(self):
		...
		thread1 = ThreadWithReturnValue(
			target=self.stripe_rendering,
			args=[-h2, 0]
		)
		thread2 = ThreadWithReturnValue(
			target=self.stripe_rendering,
			args=[0, h2]
		)
		thread1.start()
		thread2.start()
		Image[0:h2] = thread1.join()
		Image[h2:self.h] = thread2.join()
		return Image
		
	
def main():	
	plane = Plane(y=-0.5, b=3.14159)
	sphere = Sphere(x=-0.9, y=0, z=0.5, radius=0.75)
	cube = Cube(x=1, y=0.5, z=0, side=1.5, a=0.5)
	sphere_to_cut = Sphere(x=1, y=0.5, z=0)
	tor = Tor(x=0.3, y=0.75, z=-0.7, r=0.125, B_R=0.75)
